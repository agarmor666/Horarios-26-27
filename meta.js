...
(function(){
  ...
  style.textContent=`
    ...
    #apoyo-refuerzo .support-field textarea{min-height:105px;resize:vertical}#supAlumnado{min-height:190px}.support-student-help{margin-top:6px;color:#64748b;font-size:.8rem;line-height:1.35}.support-tramos{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  ...
  function selectedStudents(){const s=document.getElementById('supAlumnado');return s?[...s.selectedOptions].map(o=>o.value).filter(Boolean):[]}
  async function loadStudents(group){
    const s=document.getElementById('supAlumnado');
    s.disabled=true;s.innerHTML='<option value="">Cargando alumnado…</option>';
    if(!group){s.innerHTML='<option value="">Selecciona primero un grupo…</option>';return}
    try{
      const r=await fetch(STUDENTS_API+'?action=students-get&group='+encodeURIComponent(group),{credentials:'omit',redirect:'follow',cache:'no-store'});
      ...
      s.innerHTML=j.students.length?j.students.map(name=>'<option value="'+esc(name)+'">'+esc(name)+'</option>').join(''):'<option value="">No hay alumnado registrado para este grupo</option>';
      s.disabled=!j.students.length;
    }catch(e){
      s.innerHTML='<option value="">No se pudo cargar el alumnado</option>';
      msg('No se ha podido acceder al listado del grupo. Comprueba que has iniciado sesión con la cuenta del colegio.','err');
    }
  }function clearForm(){['supGrupo','supDocente','supArea','supFecha','supLugar','supTipo','supAlumnado','supActitud','supDificultad','supContenido','supObs'].forEach(id=>{const e=document.getElementById(id);if(e)e.value=''});document.querySelectorAll('input[name="supTramo"]').forEach(x=>x.checked=false);const students=document.getElementById('supAlumnado');if(students){students.disabled=true;students.innerHTML='<option value="">Selecciona primero un grupo…</option>'}const m=document.getElementById('supMsg');m.className='support-msg';m.textContent=''}function msg(text,type){const m=document.getElementById('supMsg');m.textContent=text;m.className='support-msg '+type}
