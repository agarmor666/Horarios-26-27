...
(function(){
  ...
  style.textContent=`
    ...
    #apoyo-refuerzo .support-field textarea{min-height:105px;resize:vertical}.support-student-help{margin-top:6px;color:#64748b;font-size:.8rem;line-height:1.35}.support-student-row{display:flex;flex-wrap:wrap;gap:7px;min-height:45px;margin-top:9px;padding:8px;border:1px dashed #94a3b8;border-radius:11px;background:#f8fafc;align-items:center}.support-student-empty{color:#64748b;font-size:.86rem}.support-student-chip{display:inline-flex;align-items:center;gap:6px;padding:7px 9px;border:1px solid #99f6e4;border-radius:999px;background:#ccfbf1;color:#115e59;font-weight:800;font-size:.84rem}.support-student-chip button{border:0;background:transparent;color:#0f766e;font:inherit;font-weight:900;cursor:pointer;padding:0 2px}.support-tramos{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  ...
  function renderSelectedStudents(){
    ...
    row.innerHTML=[...selectedStudentNames].map(name=>'<span class="support-student-chip">'+esc(name)+'<button type="button" data-remove-student="'+esc(name)+'" aria-label="Quitar '+esc(name)+'">×</button></span>').join('');
  }
  ...
  async function loadStudents(group){
    ...
    selectedStudentNames.clear();renderSelectedStudents();
    s.disabled=true;
  ...
  document.getElementById('supSeleccionados').addEventListener('click',e=>{const b=e.target.closest('[data-remove-student]');if(!b)return;selectedStudentNames.delete(b.dataset.removeStudent);renderSelectedStudents()});
