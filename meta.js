...
(function(){
  ...
  async function saveSupport(){
    ...
    const data={action:'support-set',curso:'26-27',grupo:val('supGrupo'),sesion:sesiones.join(', '),docente:val('supDocente'),area:val('supArea'),fecha:val('supFecha'),lugar:val('supLugar'),tipo:val('supTipo'),alumnado:alumnado.join(', '),actitud:actitud,dificultad:val('supDificultad'),contenido:val('supContenido'),observaciones:val('supObs')};
    if(!data.grupo||!data.docente||!data.area||!data.fecha||!data.sesion||!data.lugar||!data.tipo||!data.alumnado||!data.dificultad||!data.contenido){msg('Completa los campos obligatorios.','err');return}
